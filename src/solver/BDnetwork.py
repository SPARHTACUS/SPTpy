...
class BD_Network:
    ...
    def __init__(self, params, redFlag, b, modelName,solver_name=Solver.GRB):
    ...
    def setup(self, params, b, modelName, solver_name):
        ...
        self.SP = {t: Model('subhSPnetwork_subh' + str(b) + '_time' + str(t),
                                solver_name = solver_name) for t in params.PERIODS_PER_SUBH[b]}
    ...
    def setCouplingVarsAndCreateAuxVars(self, couplVars, couplConstrs,
        ...
        for sp in self.SP.keys():
            self.alphaIndividualSPs[sp] = self.MP.add_var(var_type = 'C',
        ...
        self.MP.add_constr(self.alphaVarMP -
                            m.xsum(self.alphaIndividualSPs[sp] for sp in self.SP.keys())>= 0,
    ...
    def add_all_comp(self, params, hydros, thermals, network, fixedVars, b, binVars):
        ...
        (couplConstrs, couplVars, alpha, beta, _,
            _0, _1, _2, _3, copyGenVars, constrCopyGenVars, alphaVarSPnetwork) = add_all_comp(
    ...
    def optimize(self, max_seconds: float = 1e12, max_nodes: int = 10000):
        ...
        self.bestSol = np.zeros(self.params.N_COMPL_VARS, dtype = 'd')
        ...
        if not(self.params.BD_SUBHORIZON_PROB) and self.b > 0:
            ...
            self.tempFeasConstrs = add_VI_based_on_ramp(self.params, self.thermals, self.b,
                                                    self.MP, self.copyOfCouplVars, self.fixedVars)
        ...
        while (self.redFlag != 1) and (innerRedFlag != 1):
            ...
            if MPstatus in (OptS.OPTIMAL, OptS.FEASIBLE):
                ...
                if gap <= self.params.INNER_BD_NETWORK_GAP:
                ...
                raise ValueError('The master problem of a backward subhorizon is infeasible')
                ...
                innerRedFlag = np.array(1, dtype = 'int')
                raise ValueError(f"MP not optimal. Status: {MPstatus}")
            ...
            if (innerRedFlag != 1) and (self.redFlag != 1):
                ...
                if SPstatus in (OptS.OPTIMAL, OptS.FEASIBLE):
                    ...
                    if (sum(self.SP[k].objective_value for k in self.SP.keys()) +
                        ...
                        self.bestSol[self.params.VARS_PER_SUBH[self.b]] =\
                    ...
                    innerRedFlag = np.array(1, dtype = 'int')
                    print(f'SP {k} not optimal: {SPstatus}', flush=True)
                    if SPstatus == OptS.INFEASIBLE:
                        self.SP[k].write(f'infeas_SP_{k}.lp')
                        self.SP[k].write(f'infes_SP_{k}.mps')
            ...
            self.log['LB'][-1], self.log['UB'][-1], self.log['gap'][-1] = lb, ub, gap

            it += 1

            if innerRedFlag != 1:
                if (it >= self.params.INNER_BD_NETWORK_MAX_IT):
                    innerRedFlag = np.array(1, dtype = 'int')
            ...
            if innerRedFlag != 1 and (self.redFlag != 1):
                ...
                for k in self.SP.keys():
                    ...
                    piIndv[self.params.GEN_CONSTRS_PER_PERIOD[self.b][k]] =\
                                                piAll[self.params.GEN_CONSTRS_PER_PERIOD[self.b][k]]

                    nonZeros = np.where(np.abs(piIndv) > 1e-6)[0]
                    constTerm = np.inner(piIndv[nonZeros], fixedGen[nonZeros])

                    lhs = self.MP.xsum(piIndv[i]*self.copyGenVars[i] for i in nonZeros)
        ...
        if gap <= -1e-3:
            ...
            raise ValueError('Negative gap in inner BD network')

        self.objective_value, self.objective_bound = ub, lb
        ...
        if gap <= self.params.INNER_BD_NETWORK_GAP or ((it >= self.params.INNER_BD_NETWORK_MAX_IT)\
                                                                            and (ub < 1e12)):
            return(OptS.OPTIMAL)
