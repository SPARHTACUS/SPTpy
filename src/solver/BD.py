...
from solver.forward import add_VI_based_on_ramp

from addCompToModel.addAllComponents import add_all_comp

from optoptions import Solver
from solver_interface.opt_model import Model, LP_Method, OptimizationStatus as OptS
...
class BD:
    ...
    def __init__(self, params, redFlag, thermals, hydros, b, modelName,solver_name=Solver.GRB):
        ...
        self.fixedVars = np.zeros(params.N_COMPL_VARS, dtype = 'd')
        self.bestSol = np.zeros(params.N_COMPL_VARS, dtype = 'd')
        self.bestSolUB = np.zeros(params.N_COMPL_VARS, dtype = 'd')
    ...
    def setup(self, params, thermals, hydros, b, modelName, solver_name):
        ...
        self.MP.max_mip_gap = params.INNER_BD_MP_GAP
        ...
        self.constrTgDispSP ={(g,t): None for g in thermals.ID for t in range(params.T)}
    ...
    def add_all_comp(self, params, hydros, thermals, network, fixedVars, b, binVars):
        ...
        (couplConstrs, couplVars, alpha, beta, alphaVarMP, copyOfMPBinVars,
                constrOfCopyOfMPBinVars, dispStat, constrTgDisp,
                    copyGenVars, constrCopyGenVars, alphaVarSPnetwork) = add_all_comp(
                                        params, hydros, thermals, network,
                                        self.MP,
                                        self.SP.MP if params.BD_NETWORK_SUBHORIZON else self.SP,
                                        self.SP.SP if params.BD_NETWORK_SUBHORIZON else self.SP, b,
                                        fixedVars,
                                        BDbinaries = True,
                                        BDnetwork = params.BD_NETWORK_SUBHORIZON,
                                        binVars = binVars)
        ...
        self.objective = self.SP.MP.objective if params.BD_NETWORK_SUBHORIZON else self.SP.objective
    ...
    def optimize(self, max_seconds: float = 1e12, max_nodes: int = 100000):
        ...
        if self.params.SOLVER == Solver.GRB:
            # Update to make suare that the RHSs are correct
            self.MP.solver.update()

        self.tempFeasConstrs = add_VI_based_on_ramp(self.params, self.thermals, self.b,
                                                            self.MP, self.copyVars, self.fixedVars)
        ...
        while (self.redFlag != 1) and (innerRedFlag != 1):
            ...
            if MPstatus in (OptS.OPTIMAL, OptS.FEASIBLE):
                ...
                if gap <= self.params.INNER_BD_GAP:
                    innerRedFlag = np.array(1, dtype = 'int')
                ...
                if (MPstatus == OptS.INFEASIBLE):
                    ...
                    raise ValueError('The MP is infeasible')
            ...
            if innerRedFlag != 1:
                ...
                if self.params.BD_NETWORK_SUBHORIZON:
                ...
                if SPstatus == OptS.OPTIMAL:
                    ...
                    if not(self.params.BD_NETWORK_SUBHORIZON):
                        sol = np.array([self.copyOfCouplVars[i].x
                                        for i in self.params.VARS_PER_SUBH[self.b]], dtype = 'd')
                    else:
                        sol = self.SP.bestSol[self.params.VARS_PER_SUBH[self.b]]

                    fixedVars[self.params.VARS_PER_SUBH[self.b]] = sol[:]

                    fixedVars[self.params.VARS_PER_SUBH[self.b]] = np.maximum(fixedVars[
                                                                self.params.VARS_PER_SUBH[self.b]],
                                                                self.params.LB_ON_COUPL_VARS[
                                                                self.params.VARS_PER_SUBH[self.b]])

                    fixedVars[self.params.VARS_PER_SUBH[self.b]] = np.minimum(fixedVars[
                                                                self.params.VARS_PER_SUBH[self.b]],
                                                                self.params.UB_ON_COUPL_VARS[
                                                                self.params.VARS_PER_SUBH[self.b]])
                    ...
                    if (SPobjVal + (self.MP.objective_value - self.alphaVarMP.x)) < ub:
                        ...
                        self.bestSol[self.params.VARS_PER_SUBH[self.b]] = fixedVars[
                                                                self.params.VARS_PER_SUBH[self.b]]

                        self.alpha = (self.alphaVar.x if not(self.params.BD_NETWORK_SUBHORIZON) else
                                                                                    self.SP.alpha)
                        self.beta = (self.betaVar.x if not(self.params.BD_NETWORK_SUBHORIZON) else
                                                                                    self.SP.beta)
                    ...
                    if SPstatus == OptS.INFEASIBLE:
                        ...
                        raise ValueError('The SP is infeasible')
            ...
            if innerRedFlag != 1:
                if (it >= self.params.INNER_BD_MAX_IT):
                    innerRedFlag = np.array(1, dtype = 'int')
            ...
            if innerRedFlag != 1 and (self.redFlag != 1):
                ...
                lhs = self.MP.xsum(pi[i]*self.copyVars[i] for i in nonZeros)
        ...
        if gap <= -1e-3:
            self.MP.write('MP.lp')
            self.SP.write('SP.lp')
            raise ValueError('Negative gap in inner BD')
        ...
        if gap <= self.params.INNER_BD_GAP or ((it >= self.params.INNER_BD_MAX_IT) and (ub < 1e12)):
            return(OptS.OPTIMAL)
