...
if lib_path is None:
    ...
    GRBgetcoeff = grblib.GRBgetcoeff
...
class SolverGurobi(Solver):
    ...
    def get_coeff(self, constrID, varID):

        coef = ffi.new("double *")

        error = GRBgetcoeff(self._model, constrID, varID, coef)

        if error != 0:
            raise Exception('Error getting the coefficient of variable in constraint')
    ...
    def optimize(self, relax: bool = False) -> OptimizationStatus:
        ...
        if self.model.lp_method == LP_Method.PRIMAL:
            ...
            self.set_int_param("Crossover", -1)
        ...
        if (self.num_int() + self.get_int_attr("NumSOS")) and (not relax):
            if status in [8, 9, 10, 11, 13, 15]:
                nsols = self.get_int_attr("SolCount")
                if not(self.model.lp_method == LP_Method.BARRIERNOCROSS):
                    self.__obj_bound = self.get_dbl_attr("ObjBound")
                else:
                    self.__obj_bound = -1e12
                if nsols >= 1:
    ...
    def get_dbl_model_attr(self, name: str) -> float:
        res = ffi.new("double *")
        error = GRBgetdblattr(self._model, name.encode("utf-8"), res)
        if error != 0:
            raise ParameterNotAvailable(
                "Error get grb double attr of model {} index {}".format(name, index)
            )
        return res[0]
    ...
    def set_str_param(self, attr: str, value: str):
        error = GRBsetstrparam(
            GRBgetenv(self._model), attr.encode("utf-8"), value.encode("utf-8")
        )

        if error != 0:
            raise ParameterNotAvailable("Error setting attribute "+attr+" in Gurobi")
        GRBupdatemodel(self._model)
